(function(){
    ...
    function taskCtrl($scope,$http,$mdDialog,$location,$timeout){
        ...
        function gettaskList(pageNum, pageSize){
            ...
            $http.post('http://localhost:8080/yoyo-server/' + 'task/getTaskListPlatform.do',{},{params:{
...
          };
       ...
       $scope.reviewTask = function(id,status){
            $scope.showConfirm = function() {
                ...
                $mdDialog.show(confirm).then(function() {
                    ...
                    $http({
                       method:'post',
                       url:'http://localhost:8080/yoyo-server/' + 'task/reviewTask.do',
                       data: $.param({
                            taskId:id,
                            activated:status,
    ...
    function taskDetailCtrl($scope,$http,$location,$mdDialog,$timeout){
        ...
        $http.post('http://localhost:8080/yoyo-server/' + 'task/getTaskById.do',{},{params:{
            taskId:$scope.taskId
        }}).success( function (data){
            if(data.errorCode == 0){
                $scope.task = data.result;
                //判断限定性别
                if($scope.task.requireGender == 3){
                    $scope.task.requireGender = "不限";
                }else if($scope.task.requireGender == 2){
                    $scope.task.requireGender = "女";
                }else if($scope.task.requireGender == 1){
                    $scope.task.requireGender = "男";
                }
               //判断是否饮酒
               if($scope.task.isDrink == 1){
                    $scope.task.isDrink = "是";
                }else if($scope.task.isDrink  == 2){
                    $scope.task.isDrink = "否";
                }
            ...
            if($scope.task.rewardType == 1){
                 switch ($scope.task.status){
                     case 1:
                     $scope.task.status = "未支付"
                     break;
                     case 2:
                     $scope.task.status = "已支付"
                     break;
                     case 3:
                     $scope.task.status = "已抢满"
                     break;
                     case 4:
                     $scope.task.status = "已完成"
                     break;
                 }
    ...
    function taskchangeCtrl($scope,$http,$location,$mdDialog,$timeout){
        ...
        $scope.changetask = function(){
            $scope.showConfirm = function() {
                ...
                $mdDialog.show(confirm).then(function() {
                    $http.post("http://localhost:8080/yoyo-server/"+"task/modifyTask.do?",{},{params:{
                        taskId:$scope.taskId,
    ...
    function TaskDealCtrl($scope,$http,$location,$mdDialog,$timeout){
        ...
        $scope.changetask = function(){
            $scope.showConfirm = function() {
                ...
                $mdDialog.show(confirm).then(function() {
                    $http.post("http://localhost:8080/yoyo-server/"+"task/dealTask.do?",{},{params:{
        ...
        $scope.addtask = function(){
            ...
            $scope.showConfirm = function() {
                ...
                $mdDialog.show(confirm).then(function() {
                    ...
                    $http.post("http://localhost:8080/yoyo-server/"+"taskCode/addtaskUser.do?",{},{params:{
                        userId:$scope.task.userId,
                        userType:$scope.task.userType
                    }}).success(function (data){
                        if(data.errorCode == 0){
